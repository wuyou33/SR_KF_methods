...
%    Method: SVD-based implementation, robust variant
...
% ------------------------------------------------------------------- 
% References:
%   Kulikova M.V., "Numerically robust SVD-based Kalman filter implementations,"
%   in Proceedings of 2018 22nd International Conference on
%   System Theory, Control and Computing (ICSTCC), Oct. 2018, pp. 170-175.
%   DOI:   10.1109/ICSTCC.2018.8540648  
% ------------------------------------------------------------------- 
% Input:
...
function [neg_LLF,hatX,hatDP] = Riccati_KF_SVDr(matrices,initials_filter,measurements)
...
function [X,DPsqrt,QP,residual,DRek,QRek] = svd_update(X,DPsqrt,QP,z,H,sqrtR)
    ...
    PreArray    = [sqrtR; DPsqrt*QP'*H';];
     ...
     residual  =  z-H*X;                                     % residual
           tol = eps(class(DRek)); invDRek = pinv(DRek,tol); % inverse only non-zero diag entries
          Gain = QP*(DPsqrt^2)*QP'*H'*QRek*invDRek*QRek';    % Kalman gain
